...
async function main() {
  ...
  if (!args.force && !explicitDate && !isLastBusinessDayOfMonth(date)) {
    console.log(`Skipped: ${date} is not the last business day of the month.`);
    return;
  }

  if (!process.env.DATABASE_URL) {
    throw new Error("DATABASE_URL is required for monthly summary generation.");
  }

  const job = await startPipelineJob({
    jobType: "summary_monthly",
    idempotencyKey: `summary:monthly:${date}`,
    force: args.force,
  ...
  try {
    const summaryArgs = ["--date", date, "--period", "monthly"];
    ...
    if (!args.skipSlack) {
      if (slackConfigured()) {
        runTsxScript("scripts/slack-deliver-summary.ts", ["--date", date, "--period", "monthly"]);
        slackDelivery = "sent";
      } else {
    ...
    await finishPipelineJob(job.jobId, { result: { date, period: "monthly", llm: args.llm, slack_delivery: slackDelivery } });
  } catch (error) {
